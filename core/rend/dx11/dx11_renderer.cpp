...
void DX11Renderer::prepareRemakeAsyncFeed()
{
	...
	auto proposedAnchor=remakeCameraAnchor;
	{
		...
		if(anchored) {
			...
			if(support.available&&!support.rejected&&(support.rotationFromLastDegrees>20||support.translationFromLast>1)) {
				...
				NOTICE_LOG(RENDERER,"Remake anchor view cut: source=%llu generation=%u rotation_from_last_deg=%.6g translation_from_last=%.6g shared_last=%u history_retired=true anchor_retained=true",
					(unsigned long long)metadata.frameId,proposedAnchor.Generation(),support.rotationFromLastDegrees,support.translationFromLast,unsigned(support.sharedLast));
			}
		}
	}
	...
	if(result==RemakeChannelResult::Published) {
		...
		if(anchored) {
			...
			NOTICE_LOG(RENDERER,"Remake observed camera: source=%llu reference_producer=%llu generation=%u origin=%.9g,%.9g,%.9g position=%.9g,%.9g,%.9g world_recovered=false projection_max_pixels=%.9g support_points=%u shared_reference=%u shared_last=%u rotation_from_last_deg=%.6g translation_from_last=%.6g bases=%u moving_points=%u",
				...
				support.rotationFromLastDegrees,support.translationFromLast,unsigned(support.bases),unsigned(support.movingPoints));
		}
