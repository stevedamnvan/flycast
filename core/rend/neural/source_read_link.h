...
inline int DirectSourceTransform(const std::vector<shil_opcode>& ops,std::size_t index) {
 ...
 for(std::size_t j=index;j-->0;) {
  ...
  if(overlaps(prior.rd)||overlaps(prior.rd2))
   return j<256&&prior.op==shop_ftrv&&prior.rd.count()==4&&overlaps(prior.rd)?static_cast<int>(j):-1;
 }
 return -1;
}
