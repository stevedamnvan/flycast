...
struct SourceRegisterRead {std::uint32_t address=0,pc=0,value=0;bool valid=false;std::uint32_t producerPc=0;};
...
inline void InvalidateSourceSqWriters() noexcept {sourceSqWriters={};sourceRegisterReads={};InvalidateSourceRamWrites();}
...
inline void ObserveSourceSqStore(std::uint32_t address,std::uint32_t pc,
 ...
 const auto readSlot=size>>8;size&=255;
