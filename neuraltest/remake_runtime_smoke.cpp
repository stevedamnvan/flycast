...
int wmain(int argc,wchar_t** argv) {
 ...
 std::optional<SceneFill> sceneFill;
 ...
 if(argc>=3&&std::wstring(argv[argc-2])==L"--scene-fill") {
  sceneFill=ParseSceneFill(argv[argc-1]);
  if(!sceneFill||!anchoredLight||templeLightRig){std::cerr<<"scene fill requires unit XYZ, radiance 0..3, anchored light and no temple rig\n";return 2;}
  argc-=2;
 }
 ...
 if(status!=REMIXAPI_ERROR_CODE_SUCCESS) { std::cerr<<"startup failed code="<<int(status)<<"\n";outcome=9; }
  ...
  D3D9PacketScene legacyScene(ownedDevice,api,liveArtifact,liveChannelAsync,omitCutoutsControl,sceneLightRadiance.value_or(3),anchoredLight,resolveTextureReference,templeLightRig,sceneLightDirection,sceneFill);
