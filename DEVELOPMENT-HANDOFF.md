...
## Current execution checkpoint (2026-09-23 handover, LOG1185, D-243)

Read this section first. It is written to be followed step by step. Everything
below it is history, not instructions.

### Goal right now

The user wants a **playable build** of the Soulcalibur RTX Remix remaster soon,
then better characters (hair, skin, blades) and no darkening, judged in that
build. Do not restart the exact-input two-run comparison (LOG1180..1184); it is
parked (D-243).

### What exists (all verified 2026-09-23)

- `neuraltest/remake_play.py` (source commit 1f58cf26e): play launcher. Starts Flycast and the Remix helper
  with no frame/time limits. Docs: `docs/neural/REMAKE-LAUNCH.md`, "Play sessions".
- Helper play switch `FLYCAST_REMAKE_HELPER_PLAY=1` (`neuraltest/remake_runtime_budget.h`,
  `RemakePlayLimits`). Only affects session workers started by the play launcher.
- Play workspace (outside Git): `C:\Game Dev\Emulators\flycast-play\soulcalibur`.
  Double-click `Play Soulcalibur (dlss5).bat` or `Play Soulcalibur (dlaa).bat`.
  It holds its own `flycast.exe`, `tools\remake-runtime-smoke.exe`,
  `remix-play.conf`, `reshade-dlss5.ini`, `reshade-dlaa.ini`.
- Lighting fix: world-space sun (default in `remake_play.py`) plus
  `remix-play.conf` (vertex colour strength 0, shrine sky textures). Evidence and
  comparison sheets: `C:\Flycast-Evidence\lighting-fix-a` (`sheet-1..3.png`).
- Smoke test `play-logs\smoke-a`: ran 5790 frames on one helper with no failure.
  It ended because the scripted input file ran out; that is expected for
  `--scripted-input` only.
...
- Never reset, clean, stash, rebase or force-push. Never delete evidence.
- Never edit files under `C:\Flycast-Evidence` that already exist (make new folders).
- Never write the user's live Remix or ReShade config outside the play workspace.
- Never claim "60 fps", "production ready" or visual approval; the user decides looks.
- Never download models or tools, or spend money, without asking the user.
- If disks get near full (check `Get-CimInstance Win32_LogicalDisk`), stop and ask.

## Previous checkpoint (2026-09-19, LOG1184): history, not instructions

Authoritative queue: `docs/neural/BACKLOG.md`. Latest checkpoint LOG1184,2026-09-19.
Verified source checkout (2026-09-19): 723c75a726f3067b7df300d0e1ce59967383e8d0,
...
LOG1175 face-stage-followup-a: eye median returned->evaluated signed-27.115,
then native-effect/final0. Raw return also soft. Next evaluation-boundary/code
and existing non-neural comparison lane; consumer provenance not proven.
...
and visual validation; no generation retry. LOG1170 exact-alpha packaging now
passes all pixel invariants. MCP8002 current stage lacks E0786 shader; next
activate/import matching capture via existing MCP, then opt-in comparison.
...
Inner helper scopes implemented; fourbuilds/1183selftests x3 pass.
...
Fresh old-helper control COMPLETE9/11, missing5370/5371; all9integrity/packet matches.
Visual differences persist; resource_reuse_review agent reviewing regions/stages.
mesh-resource-benchmark-a BOTH COMPLETE: old/reuse repeats152/2, p9516.54/14.47ms.
...
- LOG1127 compact live transport integrated behind --capture-references.
  ...
  alias proofs completed:67/67 DDS identities verified;134existing maps intact.
  ...
  LOG1137 typed fixture5mips and actualblade9mips preserved; MCP blade metallic
...
- LOG1119 serializer geometric reserve implemented; fourbuilds/three selftests
  ...
  Still159/1200remake presents; no normal/appearance acceptance. Next remaining
  packet/digest cost preserving archive lineage. Baselineexact.
...
- LOG1116 repeat1280 TERMINAL0/11,1200fresh/evaluated,steady1080fresh/maxage3,
  ...
  intended path. Next bounded late-scene normal integration/provenance check;
  resolve diagnostic reachability before launching. No normal run dispatched.
...
motion-record-cache-timing-a. Preserve temporal and accepted-history checks.
...
handle23308; evidence and host log archived in practice-remix-temple-e.
Next current character material/lighting response. User likes arena/floor;
preserve that reference. All seven earlier displacement-correction IDs absent
from this pair; retain prior fix for applicable sources,never alias variants.
See PILOT-TEMPLE-LIGHTING.md restoration guidance. No moving-combat,
performance or full appearance acceptance; guards unchanged.
...
Isolated native Practice replay verifies Mitsurugi then opponent selection;
...
Moving run finished118 captures2440..2559, missing2441/2445; all118 joins and
native composition pass, invalid motion0. Launcher exit0, baseline exact.
Reviewed2559: returned Remix skin smoother than downstream neural skin, which
adds brown grain; hair clumped upstream. Overall appearance still rejected.
Next inspect actual supplied-consumer public controls/current tuple and use
existing exact-input comparison for skin correction; preserve reflective
weapons and Remix lighting, address upstream hair separately. Then combat and
capture-free performance. No300/600 or fresh consumer-provenance acceptance.
Evidence C:/Flycast-Evidence/character-integrated-a: receipt.json,
moving-receipt.json, moving-analysis.json, renders and moving/captures.
Candidate inactive after wrapper restoration. No source changes in this slice.
...
a check. Keep cutout conservative; curved topology stability remains open.
