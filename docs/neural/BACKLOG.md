...
| FC-045 | 4 | Conditional D3D11On12 surface | Same renderer, dedicated D3D12 lists, selected only by measured route need | doing | LOG #42-#44,#52-#53,#78,#80-#84; production queue/device, wrapped input/output rings, flip-model backbuffer ring, active-render resize/minimize/restore, borderless-fullscreen roundtrip, same-renderer API-context recreation, bidirectional normal/OIT renderer switching, and bidirectional D3D11/D3D11On12 surface switching are green for Soulcalibur; monitor-move, game-load/unload, other-API switching, and device-loss matrix remains open |
...
| FC-052 | 5 | Reset/cadence wiring | Actual emulator call sites connected | doing | LOG #49,#80-#82; production render/reset/save-state and source-transition call sites are connected; context recreation, normal-DX11/DX11-OIT switches, and native-D3D11/D3D11On12 surface switches in both directions complete fresh cadence-clean intervals; live game-load/unload validation remains pending |
...
| FC-063 | 6 | Failure acceptance | No crash/stall/poison/leak | doing | LOG #35,#75-#76,#78,#80-#84; missing runtime, WARP/non-NVIDIA, and no-NGX return clean unsupported; injected feature-create, evaluate, ring/fence-busy, and device-removed-status controls retain byte-exact native fallback on D3D11 and D3D11On12, bounded holds recover, removed status latches, and active resize/minimize/borderless-fullscreen/context/renderer/surface transitions close cleanly on normal/OIT paths; real device loss, SEH, runtime removal while active, and broader transitions remain |
