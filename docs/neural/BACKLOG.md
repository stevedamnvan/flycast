...
| FC-044 | 4 | D3D11 DLAA/SR | 240-frame public NGX matrix | doing | LOG #34,#37-#40,#61,#63,#72,#94-#95; deterministic Soulcalibur combat covers target-native Auto/J/K, exact 427x320-to-640x480 Quality SR, and a 5120x3840 8x-native reference; Auto beats J on the fidelity components and K provides no material gain, so Auto remains default; confirmed external temporal and broader-title evidence remain pending for Gate 16 |
...
| FC-054 | 5 | Capture CLI | Rate-limited artifact package | doing | LOG #73-#74,#90-#95,#99; bounded production D3D11/D3D11On12 normal/OIT capture writes source/guidance/public/final/difference/flicker artifacts, locale-stable manifests, correspondence diagnostics, component metrics, capture-only per-pass GPU timings, and a provenance/status-aware comparison index; opt-in test-automation captures retain the exact input stream/hash, external confirmation remains fail-closed, and an exact emulated-frame evidence threshold can target gameplay without stalling earlier frames; the full title matrix remains |
...
| FC-065 | 6 | Manual game matrix | Legal available images; gaps stated | doing | LOG #73,#90,#94-#95; Soulcalibur has a pixel-repeatable Hoko Temple combat sequence with retained input provenance across native, target-native DLAA Auto/J/K, accepted 427x320 Quality SR, and 5120x3840 8x native; one active-combat Faithful frame has exact-input external mutation plus same-frame presentation proof; target-native Auto reduced temporal variance and retained better aggregate fidelity components than J while K provided no gain; confirmed external temporal sequences, other legal titles, and a final winner remain |
