...
| FC-026 | 2 | History generation | All structural reset sources increment | doing | LOG #20,#26,#49,#80-#82,#85; enable/mode/resize/overflow/framebuffer-source plus global render reset/save-state deserialize are wired; context, renderer, surface, and same-media reload transitions retain explicit resets and discontinuities without false output continuity; save-state runtime evidence remains pending |
...
| FC-034 | 3 | Reset rules | All cadence and scene-cut resets | doing | LOG #17,#26,#68-#69,#80-#82,#85; accepted-frame age/skips and unmatched-area scene cuts conservatively invalidate motion, increment history generation, and produce full current-color protection; renderer/context/surface and same-media reload call-site evidence is green, while save-state runtime evidence remains pending |
...
| FC-045 | 4 | Conditional D3D11On12 surface | Same renderer, dedicated D3D12 lists, selected only by measured route need | doing | LOG #42-#44,#52-#53,#78,#80-#85; production queue/device, wrapped input/output rings, flip-model backbuffer ring, active-render resize/minimize/restore, borderless-fullscreen roundtrip, same-renderer API-context recreation, bidirectional normal/OIT renderer switching, bidirectional D3D11/D3D11On12 surface switching, and same-media unload/reload are green for Soulcalibur; monitor-move, cross-title load, other-API switching, and device-loss matrix remains open |
...
| FC-052 | 5 | Reset/cadence wiring | Actual emulator call sites connected | doing | LOG #49,#80-#82,#85; production render/reset/save-state and source-transition call sites are connected; context recreation, renderer/surface switches, and same-media unload/reload are cadence-clean across normal/OIT D3D11 and D3D11On12; cross-title load and save-state runtime validation remain pending |
...
| FC-063 | 6 | Failure acceptance | No crash/stall/poison/leak | doing | LOG #35,#75-#76,#78,#80-#85; missing runtime, WARP/non-NVIDIA, and no-NGX return clean unsupported; injected feature-create, evaluate, ring/fence-busy, and device-removed-status controls retain byte-exact native fallback on D3D11 and D3D11On12, bounded holds recover, removed status latches, and active window/context/renderer/surface/same-media lifecycle transitions close cleanly on normal/OIT paths; real device loss, SEH, runtime removal while active, and broader transitions remain |
| FC-064 | 6 | Performance | Invariants and measured targets | doing | LOG #75,#77-#84; asynchronous no-flush production telemetry reports per-pass GPU P50/P95/P99, exact source/accepted/displayed frame identity, present-call intervals, repeat/drop/gap/alternation/latency counters, stage/fallback counts, query-ring pressure, and post-warmup VRAM growth; four 600-sample active resize/minimize, four borderless-fullscreen, four post-context-restart, four bidirectional post-renderer-switch, and four bidirectional post-surface-switch intervals cover normal/OIT D3D11 and D3D11On12; paired 10000-sample normal and OIT soaks are cadence-clean, with normal nonpositive VRAM growth and OIT bounded +647168/native versus flat/On12; title, external, resource-object, and D3D12-queue timing coverage remains |
