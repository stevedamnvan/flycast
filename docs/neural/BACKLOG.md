...
| FC-045 | 4 | Conditional D3D11On12 surface | Same renderer, dedicated D3D12 lists, selected only by measured route need | doing | LOG #42-#44,#52-#53,#78; production queue/device, wrapped input/output rings, flip-model backbuffer ring, and active-render resize/minimize/restore are green for normal/OIT Soulcalibur on both surfaces; fullscreen, monitor-move, renderer-restart, game-load/unload, and device-loss matrix remains open |
...
| FC-063 | 6 | Failure acceptance | No crash/stall/poison/leak | doing | LOG #35,#75-#76,#78; missing runtime, WARP/non-NVIDIA, and no-NGX return clean unsupported; injected feature-create, evaluate, ring/fence-busy, and device-removed-status controls retain byte-exact native fallback on D3D11 and D3D11On12, bounded holds recover, removed status latches, and active resize/minimize/restore closes cleanly on normal/OIT surfaces; real device loss, SEH, runtime removal while active, and broader transitions remain |
| FC-064 | 6 | Performance | Invariants and measured targets | doing | LOG #75,#77-#78; asynchronous no-flush production telemetry reports per-pass GPU P50/P95/P99, exact source/accepted/displayed frame identity, present-call intervals, repeat/drop/gap/alternation/latency counters, stage/fallback counts, query-ring pressure, and post-warmup VRAM growth; four 600-sample active-transition intervals cover normal/OIT D3D11 and D3D11On12; longer/title/external/D3D12-queue coverage remains |
