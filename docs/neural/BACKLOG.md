...
| FC-063 | 6 | Failure acceptance | No crash/stall/poison/leak | doing | LOG #35,#75-#76; missing runtime, WARP/non-NVIDIA, and no-NGX return clean unsupported; injected feature-create, evaluate, ring/fence-busy, and device-removed-status controls retain byte-exact native fallback on D3D11 and D3D11On12, bounded holds recover, and removed status latches; real device loss, SEH, runtime removal while active, and broader transitions remain |
| FC-064 | 6 | Performance | Invariants and measured targets | doing | LOG #75; asynchronous no-flush production telemetry reports per-pass GPU P50/P95/P99, present-call intervals, stage/fallback counts, query-ring pressure, and post-warmup VRAM growth; longer/title/external/D3D12-queue coverage remains |
