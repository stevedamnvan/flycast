...
| FC-054 | 5 | Capture CLI | Rate-limited artifact package | doing | LOG #73-#74,#90-#95; bounded production D3D11/D3D11On12 normal/OIT capture writes source/guidance/public/final/difference/flicker artifacts, locale-stable manifests, correspondence diagnostics, component metrics, and a provenance/status-aware comparison index; opt-in test-automation captures retain the exact input stream/hash, external confirmation remains fail-closed, and an exact emulated-frame evidence threshold can target gameplay without stalling earlier frames; capture GPU timings and the full title matrix remain |
