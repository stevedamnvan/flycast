...
| FC-067 | R | Reuse-first remake feasibility | Bounded scene packet, analytic controls, public Remix SDK adapter disposition, then separately gated moving-game reconstruction | doing | D-094, LOG #152, TRANSFORM-TA-INPUT-AUDIT.md. M1 mock/M2 export, decoder, replay and source-material sidecar implemented; one executed transform reaches an exact bulk TA input packet via arithmetic/SQ/RAM with wrong-offset control. Next actual TA context/decoded-vertex identity, then source projection semantics. Strict source equality 29/30 remains failed/parked; framebuffer-light approximation rejected. No world camera, real Remix GPU or combined presentation proven. Does not replace FC-048 or FC-056/066. |
| FC-069 | 6 | Definition of done | All non-contingent requirements green | todo | |

## Next bounded assignment -- FC-067 TA context and source semantics
