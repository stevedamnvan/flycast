...
| FC-064 | 6 | Performance | Invariants and measured targets | doing | LOG #75,#77-#89,#96; asynchronous no-flush production telemetry reports per-pass GPU P50/P95/P99, exact source/accepted/displayed frame identity, present-call intervals, repeat/drop/gap/alternation/latency counters, stage/fallback counts, query-ring pressure, post-warmup VRAM, and scoped Flycast-owned neural GPU-object counts; transition and four 10000-sample normal/OIT soaks cover both surfaces; a deterministic 600-frame combat interval now covers the supplied external consumer ON versus explicit policy-OFF with clean cadence and bounded resources; external/D3D12 queue evaluation time remains explicitly unavailable, and broader-title/device evidence remains |
