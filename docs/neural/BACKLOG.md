...
| FC-067 | R | Reuse-first remake feasibility | Bounded scene packet, analytic controls, public Remix SDK adapter disposition, then separately gated moving-game reconstruction | doing | D-095, LOG #153, TRANSFORM-DECODE-AUDIT.md. M1 mock/M2 export, decoder, replay and material sidecar implemented; one executed source transform is linked through exact TA copy to a decoded vertex with wrong-context control. Next bounded projection/model/view semantics and ambiguity checks, only necessary additional linked samples. Strict source equality 29/30 remains failed/parked; framebuffer-light approximation rejected. No world camera, real Remix GPU or combined presentation proven. Does not replace FC-048 or FC-056/066. |
| FC-069 | 6 | Definition of done | All non-contingent requirements green | todo | |

## Next bounded assignment -- FC-067 source transform semantics
