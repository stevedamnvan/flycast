...
| FC-045 | 4 | Conditional D3D11On12 surface | Same renderer, dedicated D3D12 lists, selected only by measured route need | doing | LOG #42-#44,#52-#53,#78,#80-#82; production queue/device, wrapped input/output rings, flip-model backbuffer ring, active-render resize/minimize/restore, same-renderer API-context recreation, bidirectional normal/OIT renderer switching, and bidirectional D3D11/D3D11On12 surface switching are green for Soulcalibur; fullscreen, monitor-move, game-load/unload, other-API switching, and device-loss matrix remains open |
...
| FC-063 | 6 | Failure acceptance | No crash/stall/poison/leak | doing | LOG #35,#75-#76,#78,#80-#82; missing runtime, WARP/non-NVIDIA, and no-NGX return clean unsupported; injected feature-create, evaluate, ring/fence-busy, and device-removed-status controls retain byte-exact native fallback on D3D11 and D3D11On12, bounded holds recover, removed status latches, and active window/context/renderer/surface transitions close cleanly on normal/OIT paths; real device loss, SEH, runtime removal while active, and broader transitions remain |
| FC-064 | 6 | Performance | Invariants and measured targets | doing | LOG #75,#77-#83; asynchronous no-flush production telemetry reports per-pass GPU P50/P95/P99, exact source/accepted/displayed frame identity, present-call intervals, repeat/drop/gap/alternation/latency counters, stage/fallback counts, query-ring pressure, and post-warmup VRAM growth; four 600-sample active-window-transition, four post-context-restart, four bidirectional post-renderer-switch, and four bidirectional post-surface-switch intervals cover normal/OIT D3D11 and D3D11On12; paired 10000-sample normal and OIT soaks are cadence-clean, with normal nonpositive VRAM growth and OIT bounded +647168/native versus flat/On12; title, external, resource-object, and D3D12-queue timing coverage remains |
