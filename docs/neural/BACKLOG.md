...
| FC-056 | 5 | Experimental DLSS 5 consumer mode | Route-neutral public contract, readiness ladder, native fallback, no private implementation | doing | LOG #50-#58; D3D11On12 has intercepted feature-18 activity plus frame-tagged public-output consumption; an opt-in sentinel reached all 1024 expected swapchain pixels and explicit `EnableHooks=0` retained native fallback, but no paired ON/OFF capture distinguishes external neural pixels yet |
...
| FC-066 | 6 | Mandatory DLSS 5 provenance test | User-supplied real-emulator route passes all Gate 10 items | doing | LOG #53-#58; route identity, public-contract interception, feature-18 create/evaluate, public-output frame association including frame 0, accepted blit, 1024/1024 public-output sentinel pixels, and add-on-absent/NGX-absent/explicit host-policy-off fallbacks are positive; exact feature-18-to-public-output identity, paired neural pixel differentiation, valid F6 A/B, and production latency remain open |
...
- Conditional D3D11On12/native D3D12: active; LOG #52-#58 prove stable queue-owned presentation, public-contract interception, repeated feature-18 evaluation through count 60, and a developer-only marker copied into the public output then found at 1024/1024 expected pixels in the presented swapchain backbuffer. The frame-0 edge case retains one identity through capture, blit, backbuffer verification, and Present. A later ON capture differed from its source, but its source did not match the explicit policy-OFF control, so it is not paired neural evidence. Exact external-output identity, paired neural pixel differentiation, valid F6 A/B, production latency, and the full transition matrix remain open.
